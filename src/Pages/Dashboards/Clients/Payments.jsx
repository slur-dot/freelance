...
export default function Payments() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8 lg:p-10">
      {/* Title */}
      <h1 className="text-2xl sm:text-3xl font-bold mb-6">{t('client_dashboard.payments.title')}</h1>

      {/* Stats Section */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-20">
        {/* Available Funds */}
        <div>
          <h2 className="text-lg font-bold mb-2 text-black">{t('client_dashboard.payments.stats.total_payments')}</h2>
        ...
        {/* Future Payments */}
        <div>
          <h2 className="text-lg font-bold mb-2 text-black">{t('client_dashboard.payments.stats.last_month')}</h2>
        ...
        <div>
          <h2 className="text-lg font-bold mb-2 text-black">
            {t('client_dashboard.payments.stats.summary')}
          </h2>
          <div className="bg-white p-4 rounded-lg shadow-sm h-full">
            <span className="text-sm sm:text-base">{t('client_dashboard.payments.stats.transactions')}</span>
            <h3 className="text-3xl sm:text-4xl font-bold">{stats.totalCompleted}</h3>
            <div className="space-y-2 mt-2 text-sm text-gray-600">
              <p className="text-xs text-gray-500">
                {t('client_dashboard.payments.stats.summary_desc')}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div className="flex flex-wrap gap-2">
          <button className="flex items-center gap-1 border border-gray-300 px-3 sm:px-4 py-2 rounded-md bg-white hover:bg-gray-100">
            {t('client_dashboard.payments.filters.date_range')} <ChevronDown className="h-4 w-4" />
          </button>
          <button className="flex items-center gap-1 border border-gray-300 px-3 sm:px-4 py-2 rounded-md bg-white hover:bg-gray-100">
            {t('client_dashboard.payments.filters.activity')} <ChevronDown className="h-4 w-4" />
          </button>
        </div>
      ...
      {/* Info Text */}
      <div className="text-sm text-gray-500 mb-4">{t('client_dashboard.payments.filters.showing_results', { count: 0 })}</div>
        ...
        <table className="w-full min-w-[600px] text-left text-sm">
          <thead>
            <tr className="bg-gray-50 border border-gray-100">
              <th className="py-2 px-2 sm:px-4">{t('client_dashboard.payments.table.date')}</th>
              <th className="px-2 sm:px-4">{t('client_dashboard.payments.table.activity')}</th>
              <th className="px-2 sm:px-4">{t('client_dashboard.payments.table.description')}</th>
              <th className="px-2 sm:px-4">{t('client_dashboard.payments.table.from')}</th>
              <th className="px-2 sm:px-4">{t('client_dashboard.payments.table.order')}</th>
              <th className="text-right px-2 sm:px-4">{t('client_dashboard.payments.table.amount')}</th>
            </tr>
          </thead>
          <tbody>
            {payments.length > 0 ? (
                ...
                <td colSpan={6} className="h-60 text-center">
                  <div className="flex flex-col items-center justify-center h-full">
                    <h3 className="text-lg font-bold mb-2">
                      {t('client_dashboard.payments.table.empty_title')}
                    </h3>
                    <p className="text-sm text-gray-600 w-full">
                      {t('client_dashboard.payments.table.empty_desc')}
                    </p>
                  </div>
                </td>
              </tr>

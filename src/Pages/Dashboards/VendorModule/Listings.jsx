...
export default function Listings() {
  ...
  const toggleStatus = async (listingId, nextStatus) => {
    try {
      ...
      setError(e.message);
      alert(t("failed_to_update_status_620", "Failed to update status"));
    }
  };
  ...
  const handleDelete = async (listingId) => {
    ...
    try {
      ...
      setError(e.message);
      alert(t("failed_to_delete_product_761", "Failed to delete product"));
    }
  };
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (!cleanTitle || !cleanDesc) {
        alert(t("title_and_description_cannot_contain_invalid_chara_100", "Title and description cannot contain invalid characters"));
        return;
      }
      ...
      setError(e.message);
      alert(t("failed_to_create_product_296", "Failed to create product"));
    }
  };
  ...
  if (loading) return <div className="p-6">{t("loading_315", "Loading...")}</div>;
      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t("description_401", "Description")}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t("price_33", "Price")}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t("currency_363", "Currency")}</label>
                  <select
                  ...
                  onChange={(e) => setNewProduct({ ...newProduct, currency: e.target.value })}>
                    ...
                    <option value="GNF">{t("gnf_69", "GNF")}</option>
                    <option value="USD">{t("usd_948", "USD")}</option>
                  </select>
                </div>
              </div>
              
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mt-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">{t("product_images_432", "Product Images")}</label>
                <input
                type="file"
                multiple
                ...
                <p className="text-xs text-gray-500 flex flex-col gap-1">
                  <span>{t("_maximum_5_images_879", "\u2022 Maximum 5 images")}</span>
                  <span>{t("_max_file_size_2mb_per_image_583", "\u2022 Max file size: 2MB per image")}</span>
                  <span>{t("_supported_jpg_png_webp_492", "\u2022 Supported: JPG, PNG, WEBP")}</span>
                </p>
                {images.length > 0 &&
              <div className="mt-3 bg-white p-2 border border-green-200 rounded text-sm text-green-700">
                    <span className="font-semibold">{images.length} {t("images_692", "image(s)")}</span> {t("ready_to_upload_938", "ready to upload")}
                  </div>
              }
              </div>

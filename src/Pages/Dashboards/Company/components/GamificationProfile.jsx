      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          ...
          <div key={index} className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
              <div>
                <p className="font-medium">{course.title}</p>
                <p className="text-sm text-gray-600">{course.cost.toLocaleString()} {t("gnf__206", "GNF -")} {course.status}</p>
              </div>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-3 bg-blue-50 rounded-lg">
            ...
            <p className="text-lg font-semibold">{financialBreakdown.totalSpent.toLocaleString()} {t("gnf_744", "GNF")}</p>
          </div>
          <div className="p-3 bg-purple-50 rounded-lg">
            <p className="text-sm text-gray-600">{t('company_dashboard.gamification_freelance_spending')}</p>
            <p className="text-lg font-semibold">{financialBreakdown.freelanceSpending.toLocaleString()} {t("gnf_810", "GNF")}</p>
          </div>
          <div className="p-3 bg-orange-50 rounded-lg">
            <p className="text-sm text-gray-600">{t('company_dashboard.gamification_seller_purchases')}</p>
            <p className="text-lg font-semibold">{financialBreakdown.sellerPurchases.toLocaleString()} {t("gnf_799", "GNF")}</p>
          </div>
          <div className="p-3 bg-green-50 rounded-lg">
            <p className="text-sm text-gray-600">{t('company_dashboard.gamification_training_costs')}</p>
            <p className="text-lg font-semibold">{financialBreakdown.trainingCosts.toLocaleString()} {t("gnf_84", "GNF")}</p>
          </div>
        </div>
      </div>

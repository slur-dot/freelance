...
export default function IssueReporting() {
  ...
  const handleCreateIssue = async (e) => {
    ...
    try {
      ...
      alert(t("failed_to_create_ticket_please_try_again_259", "Failed to create ticket. Please try again."));
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t("issue_reporting_336", "Issue Reporting")}</h1>
          <p className="text-gray-500 mt-1">{t("track_and_manage_it_support_tickets_and_facility_p_288", "Track and manage IT support tickets and facility problems.")}</p>
        </div>
        <button
          ...
          {t("report_new_issue_652", "Report New Issue")}
        </button>
      </div>
              ...
              <CheckCircle2 className="w-6 h-6" />}
                ...
                'bg-gray-100 text-gray-600'}`
                  ...
                  <div className="text-sm text-gray-500 mt-1 flex items-center gap-3">
                    <span>{t("reported_by_258", "Reported by")} <span className="font-medium text-gray-700">{issue.createdBy || 'Unknown'}</span></span>
          ...
          <div className="bg-white rounded-lg max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold">{t("report_new_issue_221", "Report New Issue")}</h2>
              ...
              <form onSubmit={handleCreateIssue} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t("subject__31", "Subject *")}</label>
                  <input
                  type="text"
                  value={newIssue.subject}
                  onChange={(e) => setNewIssue((prev) => ({ ...prev, subject: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500"
                  placeholder={t("brief_description_of_the_issue_593", "Brief description of the issue")}
                  required />
                
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t("description_632", "Description")}</label>
                  <textarea
                  value={newIssue.description}
                  onChange={(e) => setNewIssue((prev) => ({ ...prev, description: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500"
                  rows={4}
                  placeholder={t("provide_more_details_about_the_issue_255", "Provide more details about the issue...")} />
                
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t("category_752", "Category")}</label>
                    <select
                    value={newIssue.category}
                    onChange={(e) => setNewIssue((prev) => ({ ...prev, category: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md">
                    
                      <option value="general">{t("general_366", "General")}</option>
                      <option value="technical">{t("technical_566", "Technical")}</option>
                      <option value="billing">{t("billing_611", "Billing")}</option>
                      <option value="dispute">{t("dispute_985", "Dispute")}</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t("priority_771", "Priority")}</label>
                    <select
                    value={newIssue.priority}
                    onChange={(e) => setNewIssue((prev) => ({ ...prev, priority: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md">
                    
                      <option value="low">{t("low_410", "Low")}</option>
                      <option value="medium">{t("medium_436", "Medium")}</option>
                      <option value="high">{t("high_867", "High")}</option>
                      <option value="urgent">{t("urgent_863", "Urgent")}</option>
                    </select>
                  </div>
                </div>

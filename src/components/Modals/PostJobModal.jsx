...
export default function PostJobModal({ isOpen, onClose, user, onSave }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const newJob = await VendorService.postJob(user.uid, payload);
        ...
        <div className="p-8 space-y-5 overflow-y-auto flex-1">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder={t("eg_senior_react_developer_needed_908", "e.g. Senior React Developer Needed")}
              value={form.title}
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={4}
              placeholder={t("describe_the_project_requirements_expectations_a_940", "Describe the project requirements, expectations, and deliverables...")}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <select
                ...
                <option value="">{t("select_category_20", "Select category")}</option>
                <option value="Software Development">{t("software_development_218", "Software Development")}</option>
                <option value="Cloud & Infrastructure">{t("cloud_infrastructure_75", "Cloud & Infrastructure")}</option>
                <option value="Cyber Security">{t("cyber_security_604", "Cyber Security")}</option>
                <option value="Data & Analytics">{t("data_analytics_858", "Data & Analytics")}</option>
                <option value="SAP">{t("sap_726", "SAP")}</option>
                <option value="Business Applications">{t("business_applications_762", "Business Applications")}</option>
                <option value="Design & Creative">{t("design_creative_687", "Design & Creative")}</option>
                <option value="IT Support">{t("it_support_822", "IT Support")}</option>
                <option value="Other">{t("other_264", "Other")}</option>
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
